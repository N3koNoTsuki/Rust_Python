...
def notify(
    method_name: str = None, address: str = "unix:///var/run/arduino-router.sock"
):
    ...
    def decorator(func):
        ...
        if func_is_unsupported:
            raise TypeError(
                f"'{func.__name__}' is expected to be a function but is a method or a classmethod."
            )
        ...
        def wrapper(*args, **kwargs):
            ...
            if kwargs:
                raise TypeError(
                    f"Unexpected {list(kwargs.keys())} keyword args: only positional args are supported."
                )
...
def call(
    ...
    timeout: int | None = 10,
    address: str = "unix:///var/run/arduino-router.sock",
):
...
def provide(method_name=None, address: str = "unix:///var/run/arduino-router.sock"):
    ...
    def decorator(func):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to register method '{actual_method_name}': {e}"
            ) from e
...
class ClientServer(metaclass=SingletonMeta):
    def __init__(self, address: str = "unix:///var/run/arduino-router.sock"):
        ...
        self._read_thread = threading.Thread(
            target=self._conn_manager, name="Bridge.read_loop", daemon=True
        )
    ...
    def call(self, method_name: str, *params, timeout: int = 10):
        ...
        try:
            ...
            if success:
                ...
                raise ValueError(
                    f"Request '{method_name}' failed: {err_msg} ({err_code})"
                )
        except queue.Empty:
            ...
            with (
                self.callbacks_lock
            ):  # Ensure callback is cleaned up on any exception path
                self.callbacks.pop(msgid, None)
            raise
                    ...
                    with self.handlers_lock:
                        for method in self.handlers.keys():
                            try:
                                ...
                                logger.error(
                                    f"Failed to re-register method '{method}' after reconnection: {e}"
                                )

                if self.handlers:
                    t = threading.Thread(
                        target=register_methods_on_reconnect,
                        name="Bridge.register_methods_on_reconnect",
                        daemon=True,
                    )
                    t.start()
    ...
    def _decode_method(self, method_name: any) -> str:
        ...
        if isinstance(method_name, str):
            ...
            raise ValueError(
                f"Invalid method name type: {type(method_name)}. Expected str or bytes."
            )
    ...
    def _handle_msg(self, msg: list):
        ...
        try:
            if msg_type == 0:  # Request: [0, msgid, method, params]
                if len(msg) != 4:
                    raise ValueError(
                        f"Invalid RPC request: expected length 4, got {len(msg)}"
                    )
                _, msgid, method, params = msg
                if not isinstance(params, (list, tuple)):
                    raise ValueError(
                        "Invalid RPC request params: expected array or tuple"
                    )
                ...
                if handler:
                    try:
                        ...
                        logger.error(
                            f"Failed to run user-provided call handler for method '{method_name}': {e}"
                        )
                        self._send_response(msgid, e, None)
                else:
                    self._send_response(
                        msgid,
                        NameError(f"Method not found: '{method_name}'", method_name),
                        None,
                    )
                ...
                if len(msg) != 4:
                    raise ValueError(
                        f"Invalid RPC response: expected length 4, got {len(msg)}"
                    )
                ...
                if cbs:
                    ...
                    if result is None and error is None:
                        ...
                        if result is not None or (
                            error is not None and error[0] == ROUTE_ALREADY_EXISTS_ERR
                        ):
                            on_result(result)
                ...
                if len(msg) != 3:
                    raise ValueError(
                        f"Invalid RPC notification: expected length 3, got {len(msg)}"
                    )
                _, method, params = msg
                if not isinstance(params, (list, tuple)):
                    raise ValueError(
                        "Invalid RPC notification params: expected array or tuple"
                    )
