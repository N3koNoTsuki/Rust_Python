...
class EIPUDPHandler:
    ...
    def recv_loop(self) -> None:
        ...
        while True:
            ...
            items = CIP.parse_cpf(data)
            if len(items) < 2:
                log.warning("Less than 2 CPF items in UDP packet, ignoring.")
                continue

            item2_id, connected_data = items[1]
            if item2_id != CPF_IO_DATA:
                log.warning(f"UDP Item 1 ID is 0x{item2_id:04X}, expected 0x{CPF_IO_DATA:04X}, ignoring.")
                continue
...
async def task_send_inputs(handler: EIPUDPHandler, conn_state: dict):
    ...
    while True:
        ...
        packet = bytes(CIP.build_cpf([
            (CPF_CONNECTED_ADDR, struct.pack('<II', conn_state.get('t_o_conn_id', 0), handler.encap_seq)),
            (CPF_IO_DATA, struct.pack('<H', handler.cip_seq) + bytes([inputs]))
        ]))
